...
export const Heading = ({
  ...
  
  {
    switch (level) {
      case 1:
        return <h1 className="text-xl font-bold" id={getId(children?.toString() || '')}>{children}</h1>
      case 2:
        return <h2 className="text-lg font-bold" id={getId(children?.toString() || '')}>{children}</h2>
      case 3:
        return <h3 className="font-bold text-base" id={getId(children?.toString() || '')}>{children}</h3>
      default:
        return <h1 className="text-2xl font-bold" id={getId(children?.toString() || '')}>{children}</h1>
    }
  }
}
